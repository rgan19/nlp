...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print ("Usage on Windows:  python viterbi.py [train file] [dev.in file] [output file path]")
        print ("Usage on Linux/Mac:  python3 viterbi.py [train file] [dev.in file] [output file path]")
        sys.exit()

    q = estimateTransition(sys.argv[1])
    e_dict = train(sys.argv[1])
    result = generate_result(sys.argv[2])
    create_test_result_file(result, sys.argv[3])